...
import { Button, App, Space, Popconfirm } from 'antd';
...
const ScreenManage = () => {
  ...
  const handleExport = useCallback(async (record: API_SCREEN.IGetScreenListData) => {
    ...
    await exportData({ id, type: 'screen' }).catch(() => {
      message.info('导出失败');
    });
  }, []);
    ...
    return [
      ...column,
      {
        ...
        render: (_: any, record: API_SCREEN.IGetScreenListData) => {
          return (
            <Space>
              <Popconfirm title="是否确定删除？" onConfirm={handleDelete.bind(null, record)}>
  ...
  return (
    <ProTable
      ...
      request={async ({ current, created, ...nextParams }) => {
        return getScreenList({
          currPage: (current || 1) - 1,
          created: JSON.stringify(created) as any,
          ...nextParams,
        })
          .then((data) => {
